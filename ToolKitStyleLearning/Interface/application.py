...
# Main
root = tk.Tk()
root.wm_title("Style learning Tool")
lblText = tk.Label(root, text = "Please select input .bvh")
lblText.pack()
...
imKeyframe = tk.PhotoImage(file="res/learnStyle.gif").subsample(5,5)
btnKeyframe = tk.Button(root, text="Learn style", compound=tk.TOP, width=80, height=80, 
                      bg='white', image=imKeyframe, command = callMotionTrack)
btnKeyframe.pack(side = tk.LEFT, padx=5, pady = 5)
imExport = tk.PhotoImage(file="res/play.gif").subsample(5,5)
btnExport = tk.Button(root, text="Play animation", compound=tk.TOP, width=80, height=80, 
                      bg='white', image=imExport, command = openfile)
