...
def l1_pca(matrix):
    ...
    num_iters_max = 5 # number of iterations to run
    sparsity_lambda = 1. # sparsity parameter, lambda in the paper
    ...
    pre_scale_factor = 0.99#1/np.std(X)
    X *= pre_scale_factor
    R = X.copy() # residual
    ...
    for k in xrange(K):
        ...
        #print ('wk size', wk.shape)
        #print ('R size', R.shape)
        #print ('np size', np.tensordot(wk, R, (0, 0)).shape)
        ...
        #print ('ck',ck.size)
        # update residual      
        R -= np.outer(wk, ck).reshape(R.shape) #?No R.shape
    C = np.array(C)
    W = np.array(W).T
    ...
    for it in xrange(num_iters_max):
        # update weights
        Rflat = R.reshape(F, N*1) # flattened residual
        ...
        for admm_it in xrange(num_admm_iterations):
            ...
            #print('solve_prefactored',solve_prefactored.shape)
            print('Z',Z.shape)
            print('U',U.shape)
            #U = U.reshape(Z.shape)
            C = cho_solve(solve_prefactored, c + rho * (Z - U).reshape(c.shape)).reshape(C.shape)
            ...
            #U = U.reshape(C.shape)
            #print('Size of C+U',(C+U).shape)
            #print('Size of Lambda',Lambda.shape)
            #Lambda = Lambda.reshape(100,150)
    ...
    # undo scaling(3,10,30)
    C /= pre_scale_factor
    return W, C

def main(matrix, output_sploc_file, output_animation_file):
    rest_shape = "first" # which frame to use as rest-shape ("first" or "average")
    K = 100 # number of components # 50
    #smooth_min_dist = 0.1 # minimum geodesic distance for support map, d_min_in paper
    #smooth_max_dist = 0.7 # maximum geodesic distance for support map, d_max in paper
    num_iters_max = 5 # number of iterations to run
    sparsity_lambda = 1. # sparsity parameter, lambda in the paper
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_sploc_file', type=str, default='./output_sploc_one_leg_jump.h5',
                        help='dataset path in hdf5 format')
    parser.add_argument('--output_anim', type=str, default='./output_anim_one_leg_jump.h5',
                        help='dataset path in hdf5 format')

    args = parser.parse_args()
    
    main(generate_matrix(100,150), 
         args.output_sploc_file, 
         args.output_anim)
    '''
    w, c = l1_pca(generate_matrix(100,150))
    print w.shape
    print c.shape
