...
#Case 1: Replace xth basic of X1 by xth of X2, then save to X3
def sparse_yu_1_test(basic1, basic2, Nbasic, k):#(C1, C2, x, y, k):
    for x in xrange(1,3):
        basic3 = basic1[:] #X3 = deep copy basic1
        C1 = basic1[x][1] #Cxth of basic1
        C2 = basic2[x][1] #Cxth of baiic2
...
def plot_fully_rec(basic, Nbasic, label, my_limb = {}, save_mp4=False, Nbasic_sta = 0):
    fully_rec = 0
    if Nbasic_sta > 0:
        xmean0 = basic[0][2]
        fully_rec = xmean0[np.newaxis]
    for i in xrange(Nbasic_sta, Nbasic):
        xmeani = basic[i][2]
        print xmeani
        fully_rec = fully_rec   + np.tensordot(basic[i][0], basic[i][1], (1, 0)) + xmeani[np.newaxis]
    new_plot(fully_rec, np.arange(0), 'result5/' + label, False, True, my_limb = my_limb, save_mp4=save_mp4)    
...
def my_test_no_opt_wc1(): 
    ...
    positions, root_pos = process_file('two_cmu_retargeted/142_08.bvh', 66, 30 , 300, 700)
    positions2, root_pos2 = process_file('two_cmu_retargeted/142_15.bvh', 66, 30 , 300, 875)
    ...
    plot_fully_rec(basic1, 2, 'dtw wc1 fully_rec of M1 basic 2' , my_limb, save_mp4=True,Nbasic_sta = 1)
    plot_fully_rec(basic1, 3, 'dtw wc1 fully_rec of M1 basic 3' , my_limb, save_mp4=True,Nbasic_sta = 2)

    plot_fully_rec(basic1, 3, 'dtw wc1 fully_rec of M1 basic 1 2 3' , my_limb, save_mp4=True)
    plot_fully_rec(basic1, 2, 'dtw wc1 fully_rec of M1 basic 1 2', my_limb, save_mp4=True)
    plot_fully_rec(basic1, 1, 'dtw wc1 fully_rec of M1 basic 1', my_limb, save_mp4=True)
    plot_fully_rec(basic2, Nbasic, 'dtw wc1 fully_rec of M2 baisc 1 2 3', my_limb, save_mp4=True)

    x = 0
    basic3 = sparse_yu_1(basic1, basic2, Nbasic, k, x)
    plot_fully_rec(basic3, Nbasic, 'dtw wc1 M3 = M1 + M2 exchanging basic 1', my_limb, save_mp4=True)
    x = 1
    basic3 = sparse_yu_1(basic1, basic2, Nbasic, k, x)
    plot_fully_rec(basic3, Nbasic, 'dtw wc1 M3 = M1 + M2 exchanging basic 2', my_limb, save_mp4=True)
    x = 2
    basic3 = sparse_yu_1(basic1, basic2, Nbasic, k, x)
    plot_fully_rec(basic3, Nbasic, 'dtw wc1 M3 = M1 + M2 exchanging basic 3', my_limb, save_mp4=True)

    basic3 = sparse_yu_1_test(basic1, basic2, Nbasic, k)
    plot_fully_rec(basic3, Nbasic, 'dtw wc1 M3 = M1 + M2 exchanging basic 2 3', my_limb, save_mp4=True)
    input('stop wc1')
...
def my_mix_c():
    ...
    new_plot(sC, np.arange(0), ' M1 motion')
    new_plot(sS, np.arange(0), ' M2 motion')
    new_plot(sT, np.arange(0), ' M3 motion')

    new_plot(compensate_translation(sC, Ctran), np.arange(0), 'Compensated M1 motion')
    new_plot(compensate_translation(sS, Stran), np.arange(0), 'Compensated M2 motion')
    new_plot(compensate_translation(sT, Ttran), np.arange(0), 'Compensated M3 motion')
    ...
    rec = np.tensordot(Ws, new_C, (1, 0)) + smean[np.newaxis]
    #new_plot(compensate_translation(rec, Stran) , sidxes,'Ss component '+ str(y) )
    new_plot(rec , sidxes,'Ss component '+ str(y) +'th')

    #YU_1 svd
    new_C = svd_yu_1(Cc, Cs, x,y, k)
    c_rec = np.tensordot(Wc, new_C, (1, 0)) + cmean[np.newaxis]
    new_plot(c_rec, cidxes,'DRYU 1: Combine '+ str(x) + 'th of M1 with ' +str(y) + 'th of M2')
